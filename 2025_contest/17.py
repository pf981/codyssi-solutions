...
def get_next_positions(pos, move):
    if move == 0:
        return {pos}

    next_positions = set()
    for pos2 in m[pos]:
        next_positions.update(get_next_positions(pos2, move - 1))
...
def count_ways(pos):
    ...
    result = 0
    moves = {pos2 for move in possible_moves for pos2 in get_next_positions(pos, move)}
    for pos2 in moves:
